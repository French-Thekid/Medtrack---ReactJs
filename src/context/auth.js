...
function AuthProvider({ children }) {
  ...
  const [showForcePasswordResetModal, setShowForcePasswordResetModal] =
    useState(false)
  const [showDisassociatedModal, setShowDisassociatedModal] = useState(false)
  const [showMultipleAccess, setShowMultipleAccess] = useState(false)
    ...
    const timerId = setInterval(() => {
      const fetchRefreshToken = async () => {
        ...
        if (result.ok) {
          ...
          localStorage.setItem('UserOrgs', organizations)
        } else {
          setErrors(json)
