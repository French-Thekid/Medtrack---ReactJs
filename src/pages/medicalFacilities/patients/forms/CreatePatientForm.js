...
import { Form, FormControl, Content, Loading } from 'components'
import { Contact, EmergencyContact, Address } from './sections'
import {
  initialPatient,
  createPatientSchema,
  Genders,
...
import { CREATE_PATIENT } from '../mutations'
import { LIST_PATIENTS } from '../queries'
import { useMutation } from '@apollo/react-hooks'

export default function CreatePatientForm({ close, showNotificationCreate }) {
  ...
  return (
    <Formik
      ...
      validationSchema={createPatientSchema({ update: false })}
      onSubmit={async (values, actions) => {
        if (values.avatar !== null) values.avatar = values.avatar.split(',')[1]
        values.emergencyContact.firstName =
          values.emergencyContact.fullName.split(' ')[0]

        values.emergencyContact.lastName =
          values.emergencyContact.fullName.split(' ')[1]
        ...
        return (
          <Form.StepForm {...props}>
            ...
            <Form.Step>
              ...
              {createPatientFailed && (
                <Content.Alert type="error" message="Fail to create patient" />
              )}{' '}
              <FormControl.Section marginTop="10px">
                <FormControl.Input
