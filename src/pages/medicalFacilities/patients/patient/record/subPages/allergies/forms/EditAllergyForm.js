...
import { UPDATE_ALLERGY } from '../mutations'
import { useMutation } from '@apollo/react-hooks'
import { createAllergySchema } from './initialValues'
import { Core, FormControl, Loading, Content } from 'components'
import { useRouteMatch } from 'react-router-dom'

export default function EditAllergyForm({
  close,
  showNotificationAllergyEdit,
}) {
  const {
    params: { patientId },
  } = useRouteMatch()
  ...
  const initialAllergy =
    JSON.parse(localStorage.getItem('selectedAllergy')) || {}

  //Mutation
  const [updateAllergy, { loading, error }] = useMutation(UPDATE_ALLERGY, {
    refetchQueries: () => [
      {
    ...
    <Formik
      ...
      onSubmit={async (values, actions) => {
        await updateAllergy({
          variables: {
            allergy: {
              id: parseInt(initialAllergy.allergyId),
              name: values.name,
      ...
      {(props) => {
        ...
        return (
          ...
          >
            {loading && <Loading />}
            {error && (
              <Content.Alert type="error" message="Fail to update allergy" />
            )}
