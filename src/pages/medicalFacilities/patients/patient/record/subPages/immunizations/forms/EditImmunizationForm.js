...
export default function CreateImmunizationForm({
  ...
  const initialImmunization =
    JSON.parse(localStorage.getItem('selectedImmunization')) || {}

  //Mutation
  const [updateImmunizationsVaccination, { loading, error }] = useMutation(
    UPDATE_IMMUNIZATION,
    {
      refetchQueries: () => [
        {
      ...
      onCompleted(updateImmunizationsVaccination) {
        showNotificationVaccineEdit()
      },
    }
  )
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, actions) => {
        await updateImmunizationsVaccination({
          variables: {
            immunizationsVaccination: {
              id: parseInt(initialImmunization.id),
              name: values.name,
              type: values.type,
        ...
        return (
          ...
          >
            ...
            {error && (
              <Content.Alert
                type="error"
                message="Fail to update immunization/vaccine"
              />
            )}
