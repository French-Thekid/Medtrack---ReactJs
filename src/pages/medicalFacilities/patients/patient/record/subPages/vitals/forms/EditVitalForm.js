...
import { UPDATE_VITALS } from '../mutations'
import { useMutation } from '@apollo/react-hooks'
import { createVitalSchema } from './initialValues'
import { Core, FormControl, Content, Loading } from 'components'
import { useRouteMatch } from 'react-router-dom'

export default function EditVitalForm({ close, showNotificationVitalEdit }) {
  const {
    params: { patientId },
  } = useRouteMatch()

  const initialVital = JSON.parse(localStorage.getItem('selectedVital')) || {}

  //Mutation
  const [updateVital, { loading, error }] = useMutation(UPDATE_VITALS, {
    refetchQueries: () => [
      {
        query: LIST_VITALS,
    ...
    onCompleted(updateVital) {
      showNotificationVitalEdit()
    },
  })
      ...
      {(props) => {
        ...
        return (
          ...
          >
            {loading && <Loading />}
            {error && (
              <Content.Alert type="error" message="Fail to update vital" />
            )}
