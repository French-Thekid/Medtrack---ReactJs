...
import { UPDATE_MEDICINE } from '../mutations'
import { useMutation } from '@apollo/react-hooks'
import { createMedicineSchema } from './initialValues'
import { Core, FormControl, Content, Loading } from 'components'
import { useRouteMatch } from 'react-router-dom'

export default function EditMedicineForm({
  close,
  showNotificationMedicineEdit,
}) {
  const {
    params: { patientId },
  } = useRouteMatch()
  ...
  const initialMedicine =
    JSON.parse(localStorage.getItem('selectedMedicine')) || {}

  //Mutation
  const [updateMedicine, { loading, error }] = useMutation(UPDATE_MEDICINE, {
    refetchQueries: () => [
      {
  ...
  return (
    <Formik
      ...
      onSubmit={async (values, actions) => {
        await updateMedicine({
          variables: {
            medicine: {
              id: parseInt(initialMedicine.medicineId),
              name: values.name,
    ...
    >
      {(props) => {
        ...
        let { name, dosage, description, status } = values
        ...
        return (
          ...
          >
            {loading && <Loading />}
            {error && (
              <Content.Alert type="error" message="Fail to update medicine" />
            )}
