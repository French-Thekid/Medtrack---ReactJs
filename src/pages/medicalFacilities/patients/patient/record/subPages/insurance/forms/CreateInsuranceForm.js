...
import { CREATE_INSURANCE } from '../mutations'
import { useMutation } from '@apollo/react-hooks'
import { initialInsurance, createInsuranceSchema } from './initialValues'
import { Core, FormControl, Content, Loading } from 'components'
import { useRouteMatch } from 'react-router-dom'

export default function CreateInsuranceForm({
  close,
  showNotificationInsuranceCreate,
}) {
  const {
    params: { patientId },
  } = useRouteMatch()
  ...
  //Mutation
  const [createMedicalInsurance, { loading, error }] = useMutation(
    CREATE_INSURANCE,
    {
      refetchQueries: () => [
        {
      ...
      onCompleted(createMedicalInsurance) {
        showNotificationInsuranceCreate()
      },
    }
  )
        ...
        return (
          ...
          >
            ...
            {error && (
              <Content.Alert
                type="error"
                message="Fail to add medical insurance"
              />
            )}
