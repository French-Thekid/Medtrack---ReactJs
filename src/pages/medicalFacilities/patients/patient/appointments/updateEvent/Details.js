...
import { Icons, FormControl, Core, Colours, Content, Loading } from 'components'
...
export default function Details({
  ...
  if (selectedDoctor !== undefined) {
    availableTime = GetDoctorAvailability(selectedDoctor, selectedDay)
  }
  ...
  return (
    ...
    >
      ...
      >
        <Core.Text>
          Please update the information below then hit save to update
          appointment
        </Core.Text>
      </div>
      <Container>
                ...
                ({ start, end, available, id }, index) => {
                  return (
                    <TimeCard
                      available={available}
                      key={index}
                      time={updated ? start : AddFiveHours(start).toISOString()}
...
const TimeCard = ({
  ...
  return (
    <div
      ...
      onClick={() => {
        console.log(time)
        if (available && selectedtime.time.split('T')[1] !== time.split('T')[1])
          setSelectedTime({ time, endTime, status: true, availabilityId: id })
      }}
      onDoubleClick={() => {
        if (!available) console.log('Well Damn')
      }}
    >
      {updated
        ? TimeExtrator.format(Date.parse(AddFiveHours(time)))
            .toLowerCase()
            .split(' ')
            .join('')
        : TimeExtrator.format(Date.parse(time))
            .toLowerCase()
            .split(' ')
            .join('')}
    </div>
