...
import { CREATE_PHARMACY } from './mutations'
import { LIST_PHARMACY } from './queries'

export default function CreateOrganisationForm({ close }) {
  const [createFacility, { loading, error: createPharmacyFailed }] =
    useMutation(CREATE_PHARMACY, {
      refetchQueries: () => [
        {
          query: LIST_PHARMACY,
        },
          ...
          <Form.StepForm {...props}>
            ...
            <Form.Step>
              ...
              {createPharmacyFailed && (
                <Content.Alert
                  type="error"
                  message={createPharmacyFailed.message}
                />
              )}
              <FormControl.FieldSet>
                <FormControl.Legend>
